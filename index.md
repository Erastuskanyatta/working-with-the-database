...
If You see the image similar to the one below, the Xampp was successfully installed in you machine.
...
### Step 2: Creating tables in MariaDB using SQL commands in terminal.
...
```
The terminal will ask you for password, just click enter as we have not set any password;
...
To confirm that you have successfully created database school, use this command;
```bash
show databases;
```
...
So far, we have created a `database school` and we have also created `tables`inside school database via terminal. Now lets view this database school and its tables in the MariaDB. When you open the URL: http://localhost/phpmyadmin/  you should have an image like this.
...
$ sudo mkdir test
```
Lets now navigate inside ``test`` folder and create two files. Use this command to navigate inside ``test`` folder;
```bash
$ cd test
```
...
$ sudo nano index.php
```
After clicking enter, a page  with a title `index.php` will be opened. Since this is not the best place to have our `index.php` code we will close this page and use another suitable code editor. To close click(ctrl+O)followed by (ctrl+enter) followed by(ctrl+x). This will take you back to the terminal.
...
To fetch data that we have saved, we will add some code inside ``index.php``. The code will added below ``</form>``.
```php
//
<br> <br>
...
### Conclusion
From this article we have learned that working with database is as easier as working with any other code. and we have come to conclusion that, it is a myth to say that Database Is only meant for backend expert only.
